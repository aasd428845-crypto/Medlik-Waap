...
-- Medlik-Waap
--
-- الهدف: تحويل التحصيل من payment تشغيلي فقط إلى سند قبض مالي
-- مرتبط بصندوق/بنك وقيد دفتر أستاذ. لا يتم ترحيل التاريخ القديم تلقائياً.
-- =============================================================
...
-- ترحيل سند قبض بصورة ذرية:
-- مدين = الصندوق/البنك، دائن = الذمم المدينة.
-- لا ينشأ payment التشغيلي إلا بعد نجاح القيد.
...
begin
  ...
  select receivable_account_id into v_receivable_account
  ...
  insert into public.financial_documents (
    ...
    v_receipt.receipt_number::text,
    coalesce(v_receipt.notes, 'Customer receipt'), auth.uid()
  ...
  insert into public.financial_journal_lines (
    ...
    v_journal_id, 2, v_receivable_account, v_branch_id,
  ...
  -- بعد نجاح القيد ننشئ payment تشغيلياً. Trigger المرحلة 0006
  -- يحافظ على current_balance وحالة الفاتورة.
  insert into public.payments (invoice_id, client_id, amount, financial_document_id)
...
-- لا نسمح باستدعاء الترحيل مباشرة من دور مجهول/عام.
-- الوصول يتم لاحقاً من Edge Function/واجهة مالية مصادق عليها.
-- =============================================================
