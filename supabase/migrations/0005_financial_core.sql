-- =============================================================
-- Migration 0005 — Financial Core / Accounting Foundation
-- Medlik-Waap — General Manager Web Dashboard
--
-- الهدف: إنشاء نواة محاسبية مؤسسية دون استبدال invoices/payments
-- الحالية. هذه migration لا تنشئ نظام فواتير أو مدفوعات موازياً.
-- =============================================================

create extension if not exists pgcrypto;

-- -------------------------------------------------------------
-- 1. الشركات والسنوات والفترات والعملات
-- -------------------------------------------------------------
...
insert into public.financial_currencies (code, name, symbol, decimal_places)
values ('YER', 'Yemeni Riyal', '﷼', 2)
on conflict (code) do nothing;

alter table public.financial_companies
  add constraint financial_companies_base_currency_fk
  foreign key (base_currency_code) references public.financial_currencies(code);
...
create index if not exists financial_documents_source_idx
  on public.financial_documents(source_table, source_id);
...
create table if not exists public.financial_journal_entries (
  ...
  fiscal_period_id uuid references public.financial_periods(id) on delete restrict,
...
create index if not exists financial_journal_entries_date_idx
  on public.financial_journal_entries(company_id, entry_date);
create index if not exists financial_journal_lines_account_idx
  on public.financial_journal_lines(account_id);
create index if not exists financial_journal_lines_branch_idx
  on public.financial_journal_lines(branch_id);

-- -------------------------------------------------------------
-- 7. وظيفة آمنة لترحيل القيد بعد التحقق من التوازن والفترة
-- -------------------------------------------------------------
...
as $$
declare
  v_entry public.financial_journal_entries%rowtype;
...
begin
  ...
  if v_entry.fiscal_period_id is null or v_period_status <> 'open' then
    raise exception 'Journal entry period is missing or closed';
  end if;
...
-- -------------------------------------------------------------
-- 8. سجل التدقيق المالي
-- -------------------------------------------------------------
create table if not exists public.financial_audit_logs (
  id uuid primary key default gen_random_uuid(),
  company_id uuid references public.financial_companies(id) on delete set null,
...
-- القراءة المالية: المدير العام والمحاسب. تفاصيل الفرع ستُقيّد في مراحل التشغيل الخاصة بالفرع.
-- نستخدم current_user_role() الموجودة في المشروع بدلاً من اختراع نظام صلاحيات آخر.

create policy "financial_companies_read" on public.financial_companies
  for select using (public.current_user_role() in ('company_director','accountant'));

create policy "financial_currencies_read" on public.financial_currencies
  for select using (auth.role() = 'authenticated');

create policy "financial_exchange_rates_read" on public.financial_exchange_rates
  for select using (public.current_user_role() in ('company_director','accountant'));

create policy "financial_years_read" on public.financial_fiscal_years
  for select using (public.current_user_role() in ('company_director','accountant'));

create policy "financial_periods_read" on public.financial_periods
  for select using (public.current_user_role() in ('company_director','accountant'));

create policy "financial_cost_centers_read" on public.financial_cost_centers
  for select using (public.current_user_role() in ('company_director','accountant'));

create policy "financial_accounts_read" on public.financial_accounts
  for select using (public.current_user_role() in ('company_director','accountant'));

create policy "financial_cash_accounts_read" on public.financial_cash_accounts
  for select using (public.current_user_role() in ('company_director','accountant'));

create policy "financial_bank_accounts_read" on public.financial_bank_accounts
  for select using (public.current_user_role() in ('company_director','accountant'));

create policy "financial_documents_read" on public.financial_documents
  for select using (public.current_user_role() in ('company_director','accountant'));

create policy "financial_journal_entries_read" on public.financial_journal_entries
  for select using (public.current_user_role() in ('company_director','accountant'));

create policy "financial_journal_lines_read" on public.financial_journal_lines
  for select using (public.current_user_role() in ('company_director','accountant'));
...
-- الكتابة المباشرة للنواة محصورة في المدير العام والمحاسب؛ الترحيل يتم عبر الدالة الآمنة.
create policy "financial_companies_write" on public.financial_companies
  for all using (public.current_user_role() = 'company_director')
  with check (public.current_user_role() = 'company_director');

create policy "financial_exchange_rates_write" on public.financial_exchange_rates
...
create policy "financial_years_write" on public.financial_fiscal_years
  for all using (public.current_user_role() = 'company_director')
  with check (public.current_user_role() = 'company_director');
...
create policy "financial_cost_centers_write" on public.financial_cost_centers
  for all using (public.current_user_role() in ('company_director','accountant'))
  with check (public.current_user_role() in ('company_director','accountant'));

create policy "financial_accounts_write" on public.financial_accounts
  for all using (public.current_user_role() in ('company_director','accountant'))
  with check (public.current_user_role() in ('company_director','accountant'));

create policy "financial_cash_accounts_write" on public.financial_cash_accounts
  for all using (public.current_user_role() in ('company_director','accountant'))
  with check (public.current_user_role() in ('company_director','accountant'));

create policy "financial_bank_accounts_write" on public.financial_bank_accounts
  for all using (public.current_user_role() in ('company_director','accountant'))
  with check (public.current_user_role() in ('company_director','accountant'));

create policy "financial_documents_write" on public.financial_documents
  for insert with check (public.current_user_role() in ('company_director','accountant'));

create policy "financial_journal_entries_write" on public.financial_journal_entries
  for insert with check (public.current_user_role() in ('company_director','accountant'));

create policy "financial_journal_lines_write" on public.financial_journal_lines
  for insert with check (public.current_user_role() in ('company_director','accountant'));

create policy "financial_audit_logs_insert" on public.financial_audit_logs
  for insert with check (public.current_user_role() in ('company_director','accountant'));

-- لا توجد سياسة UPDATE/DELETE للقيود المرحّلة؛ التصحيح سيكون عبر قيد عكسي في مرحلة القيود التشغيلية.
-- =============================================================
