...
-- مبني فوق migrations 0001..0005 الحالية ولا يستبدلها.
-- =============================================================
...
values ('YER','الريال اليمني','﷼',2),('USD','الدولار الأمريكي','$',2),('SAR','الريال السعودي','﷼',2)
on conflict (code) do nothing;

alter table public.financial_companies drop constraint if exists financial_companies_base_currency_fk;
alter table public.financial_companies add constraint financial_companies_base_currency_fk
...
create table if not exists public.financial_fiscal_years (
  ...
  created_at timestamptz not null default now(),
  check(end_date >= start_date), unique(company_id,year_code)
);
...
a lter table public.financial_companies enable row level security;
