...
function ProtectedRoute({ allowedRoles }: { allowedRoles: string[] }) {
  ...
  if (!allowedRoles.includes(userProfile.role)) return <div className="h-screen flex flex-col items-center justify-center p-4 text-center"><h2 className="text-2xl font-bold text-destructive mb-2">عذراً، غير مصرح لك</h2><p className="text-muted-foreground">ليس لديك الصلاحيات الكافية لعرض هذه الصفحة.</p><Navigate to="/login" replace /></div>;
  return <Outlet />;
}
...
function App() {
  ...
  return <AuthProvider><BrowserRouter basename={basename}><Routes>
