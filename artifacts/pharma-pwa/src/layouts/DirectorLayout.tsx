...
import { Pill, LayoutDashboard, Users, UserCheck, Truck, BarChart3, Database, Activity, LayoutList, BadgePercent, Gift, AlarmClock, Wallet, LogOut, CircleDollarSign } from 'lucide-react';
...
export function DirectorLayout() {
  ...
  const getPageTitle = () => {
    if (location.pathname.includes('/financial')) return 'الإدارة المالية';
  ...
  const navItems = [
    ...
    { to: '/director/financial', label: 'الإدارة المالية', icon: CircleDollarSign },
      ...
      <div className="flex flex-1 bg-muted/20 overflow-hidden">
        <aside className="w-64 bg-sidebar border-l border-sidebar-border flex flex-col hidden md:flex shrink-0">
          <div className="p-6 border-b border-sidebar-border">
            <div className="flex items-center gap-3">
              <div className="bg-primary/10 p-2 rounded-lg text-primary"><Pill className="w-6 h-6" /></div>
              <div>
          ...
          <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
                className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-primary/10 text-primary' : 'text-sidebar-foreground hover:bg-muted'}`}
              >
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          <header className="bg-card border-b h-16 flex items-center justify-between px-6 shrink-0 shadow-sm">
            ...
            <div className="md:hidden flex items-center"><button onClick={logout} className="text-destructive font-medium text-sm">خروج</button></div>
          </header>

          <div className="md:hidden flex items-center justify-around bg-card border-t py-2 shrink-0 fixed bottom-0 w-full z-10">
            {navItems.slice(0, 5).map((item) => (
              <NavLink key={item.to} to={item.to} className={({ isActive }) => `flex flex-col items-center gap-1 p-2 text-xs ${isActive ? 'text-primary' : 'text-muted-foreground'}`}>
                <item.icon className="w-5 h-5" />
                {item.label.split(' ')[0]}
              </NavLink>
            ))}
          ...
          <main className="flex-1 overflow-auto p-4 md:p-6 pb-20 md:pb-6 relative"><Outlet /></main>
        </div>
      </div>
    </div>
  );
